...
def pred_point(cfg: Box):
    ...
    for image_id in tqdm(dataset.image_ids):
        ...
        points = []
        for i in range(space_y//2, H-space_y//4, space_y):
            for j in range(space_x//2, W-space_x//4, space_x):
                points.append([[j, i]])
        ...
        points = torch.as_tensor(points, device=model.model.device)
        labels = torch.as_tensor(point_labels, dtype=torch.int, device=model.model.device)

        transformed_point = predictor.transform.apply_coords_torch(points, image.shape[:2])
        predictor.set_image(image)
        masks, _, _ = predictor.predict_torch(
